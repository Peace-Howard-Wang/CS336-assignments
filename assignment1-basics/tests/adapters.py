...
def run_embedding(
    ...
    stat_dict = {"weights" : weights}
    embedding.load_state_dict(stat_dict)
...
def run_transformer_block(
    ...
    state_dict ={
        "attention.wq.weights":weights["attn.q_proj.weight"],
        "attention.wk.weights":weights["attn.k_proj.weight"],
        "attention.wv.weights":weights["attn.v_proj.weight"],
        "attention.wo.weights": weights["attn.output_proj.weight"],
...
def run_transformer_lm(
    ...
    # --- 处理 state_dict 让它匹配你的 TransformerLM ---
    lm_state_dict = {}
    # token embedding
    lm_state_dict["token_embedding.weights"] = weights["token_embeddings.weight"]
    # transformer blocks
    for layer_idx in range(num_layers):
        prefix_old = f"layers.{layer_idx}."
        prefix_new = f"transfomer_blocks.{layer_idx}."

        lm_state_dict[f"{prefix_new}attention.wq.weights"] = weights[f"{prefix_old}attn.q_proj.weight"]
        lm_state_dict[f"{prefix_new}attention.wk.weights"] = weights[f"{prefix_old}attn.k_proj.weight"]
        lm_state_dict[f"{prefix_new}attention.wv.weights"] = weights[f"{prefix_old}attn.v_proj.weight"]
        lm_state_dict[f"{prefix_new}attention.wo.weights"] = weights[f"{prefix_old}attn.output_proj.weight"]

        lm_state_dict[f"{prefix_new}norm1.weights"] = weights[f"{prefix_old}ln1.weight"]
        lm_state_dict[f"{prefix_new}norm2.weights"] = weights[f"{prefix_old}ln2.weight"]

        lm_state_dict[f"{prefix_new}ffn.w1.weights"] = weights[f"{prefix_old}ffn.w1.weight"]
        lm_state_dict[f"{prefix_new}ffn.w2.weights"] = weights[f"{prefix_old}ffn.w2.weight"]
        lm_state_dict[f"{prefix_new}ffn.w3.weights"] = weights[f"{prefix_old}ffn.w3.weight"]

    # final norm & lm head
    lm_state_dict["norm.weights"] = weights["ln_final.weight"]
    lm_state_dict["output_embedding.weights"] = weights["lm_head.weight"]
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return train_bpe(input_path, vocab_size, special_tokens, True)
